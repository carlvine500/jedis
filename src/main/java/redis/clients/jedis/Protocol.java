...
public final class Protocol {
  ...
  /*
   * private static Object process(final RedisInputStream is) { final byte b = is.readByte(); if (b
   * == PLUS_BYTE) { return processStatusCodeReply(is); } else if (b == DOLLAR_BYTE) { return
   * processBulkReply(is); } else if (b == ASTERISK_BYTE) { return processMultiBulkReply(is); } else
   * if (b == COLON_BYTE) { return processInteger(is); } else if (b == MINUS_BYTE) {
   * processError(is); return null; } else { throw new JedisConnectionException("Unknown reply: " +
   * (char) b); } }
   */
